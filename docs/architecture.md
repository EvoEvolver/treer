...
`permissions=0`, and `nav=0` so rooms, New Chat, and the RC account
chrome stay hidden (Thread is a Treer Agent) while the explorer and
composer remain. AIS `ui_path` stays `/` so Proxy asset tunneling
