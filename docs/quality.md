...
| Host supervision | nohup default and PID identity, explicit systemd selection, partial-unit repair, stale-registration cleanup, and TUI/Web diagnostics | No normal macOS lifecycle CI; no Apple container machine setup CI |
...
| ACP runtime | Fake-harness AIS tests, Host `treer ui --dir` install, `just treer-acp-e2e`, and mocked web e2e for `kind=acp` create, session import, iframe flags, and machine-page UI install | No live grok/codex harness e2e in this slice |
...
| `treer-acp` / Host thread UI | `cargo test -p treer-acp -p treer-cli -p treer-agent-server -p treer-protocol`, `just treer-acp-e2e` (fake ACP), and `web/e2e` create/import/iframe tests |
