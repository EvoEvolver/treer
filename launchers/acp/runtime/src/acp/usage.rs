...
use crate::types::{home_dir, now_rfc3339};
...
impl CodexUsageReader {
    pub(super) fn new(session_id: &str) -> Self {
        let mut reader = Self {
            ...
            session_id: crate::import_id::raw_session_id(session_id).to_string(),
            path: None,
            offset: 0,
    ...
    fn read(&mut self, baseline_only: bool) -> Vec<Value> {
        ...
        if self.path.is_none() && self.next_lookup <= std::time::Instant::now() {
            ...
            self.path = find_codex_rollout(&self.home, &self.session_id);
        }
...
fn find_codex_rollout(home: &std::path::Path, session_id: &str) -> Option<PathBuf> {
