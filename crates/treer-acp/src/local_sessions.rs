...
use crate::types::{now_rfc3339, truncate_title, HistoryItem, ImportedTurn, SessionCandidate};
...
pub fn list_local_sessions(homes: &LocalSessionHomes, agent_id: &str) -> Vec<SessionCandidate> {
...
fn list_codex_sessions(home: &Path) -> Vec<SessionCandidate> {
    let mut out = Vec::new();
...
fn find_codex_session(home: &Path, session_id: &str) -> Option<SessionCandidate> {
    for sqlite in codex_state_files(home) {
        if let Ok(conn) = open_readonly(&sqlite) {
    ...
    find_codex_rollout(home, session_id).map(|path| {
        parse_codex_rollout(&path).unwrap_or_else(|| SessionCandidate {
...
) -> rusqlite::Result<Vec<SessionCandidate>> {
...
fn load_codex_history(home: &Path, session_id: &str, cwd: Option<&str>) -> Vec<ImportedTurn> {
...
fn merge_codex_turns(
    paginated: Vec<ImportedTurn>,
    rollout: Vec<ImportedTurn>,
) -> Vec<ImportedTurn> {
...
fn stable_sort_history_items(items: Vec<HistoryItem>) -> Vec<HistoryItem> {
...
fn load_codex_paginated_history(home: &Path, session_id: &str) -> Option<Vec<ImportedTurn>> {
    ...
    let mut item_stmt = conn
        ...
        .ok()?;
    let mut items_by_turn: HashMap<String, Vec<HistoryItem>> = HashMap::new();
    ...
    Some(
        turns
            .into_iter()
            .map(|(id, status, error, started)| ImportedTurn {
                id: id.clone(),
...
fn parse_codex_rollout(path: &Path) -> Option<SessionCandidate> {
    ...
    let mut current: Option<ImportedTurn> = None;
...
fn empty_codex_turn(id: String, started_at: Option<String>) -> ImportedTurn {
    ImportedTurn {
        id,
        started_at,
...
fn ensure_codex_turn<'a>(
    current: &'a mut Option<ImportedTurn>,
...
fn finish_codex_turn(current: &mut Option<ImportedTurn>, turns: &mut Vec<ImportedTurn>) {
    if let Some(turn) = current.take().filter(|turn| !turn.items.is_empty()) {
        turns.push(turn);
    }
}

fn list_grok_sessions(home: &Path) -> Vec<SessionCandidate> {
...
fn find_grok_session(home: &Path, session_id: &str) -> Option<SessionCandidate> {
...
fn parse_grok_summary(path: &Path, with_history: bool) -> Option<SessionCandidate> {
    let raw = fs::read_to_string(path).ok()?;
...
fn parse_grok_history(path: &Path) -> Vec<ImportedTurn> {
...
fn list_claude_sessions(home: &Path) -> Vec<SessionCandidate> {
...
fn find_claude_session(home: &Path, session_id: &str) -> Option<SessionCandidate> {
...
fn parse_claude_jsonl(path: &Path, with_history: bool) -> Option<SessionCandidate> {
    let raw = fs::read_to_string(path).ok()?;
...
fn dedupe_sessions(sessions: Vec<SessionCandidate>) -> Vec<SessionCandidate> {
    let mut out = Vec::new();
    for session in sessions {
        if out.iter().any(|existing: &SessionCandidate| {
...
fn item(id: String, kind: &str, text: String, status: &str, turn_id: &str) -> HistoryItem {
    HistoryItem {
        id,
...
mod tests {
    ...
    fn merges_paginated_history_with_rollout_only_items_and_turns() {
        ...
        let paginated = vec![ImportedTurn {
            id: "turn-rich".into(),
