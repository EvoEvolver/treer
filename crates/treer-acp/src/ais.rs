...
pub async fn serve(mut config: AisConfig) -> Result<AisServer> {
    ...
    if let Some(ui_dist) = &config.ui_dist {
        router = router.fallback_service(ServeDir::new(ui_dist));
    }
...
fn harness_runtime(config: &AisConfig) -> Result<(crate::acp::AcpAgentDef, AcpRuntime)> {
    match &config.harness {
        ...
        HarnessSpec::Command(command) => {
            let runtime = AcpRuntime::catalog(Some(command.clone()), config.startup_timeout_ms);
            Ok((runtime.agent_def(Some("custom"))?, runtime))
        }
        HarnessSpec::Named(name) => {
            let runtime = AcpRuntime::catalog(None, config.startup_timeout_ms);
            Ok((runtime.agent_def(Some(name))?, runtime))
        }
    }
}
