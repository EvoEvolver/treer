...
use crate::types::{now_rfc3339, BoundSession, HistoryItem, ImportedTurn};
...
impl Journal {
    ...
    pub fn seed_imported_turns(&self, turns: &[ImportedTurn]) -> Result<()> {
        for turn in turns {
            for item in &turn.items {
                self.upsert_entry(&history_to_entry(item))?;
            }
        }
        Ok(())
    }
