...
use treer_acp::{
    bind_local_session, default_state_dir, serve, AisConfig, HarnessSpec, Journal,
    LocalSessionHomes,
};

struct Running {
    _dir: TempDir,
    server: treer_acp::AisServer,
    base: String,
...
#[test]
fn bind_local_session_seeds_journal() {
    let dir = TempDir::new().unwrap();
    ...
    .unwrap();
    assert_eq!(bound.harness, "grok");
    assert_eq!(bound.session_id, "01a0513a-7417-7553-8c77-399316ec7a9b");
    assert_eq!(bound.cwd, cwd);
    let entries = journal.entries().unwrap();
    assert!(
        entries
            .iter()
            .any(|entry| entry.role.as_deref() == Some("user")
                && entry.content == json!("hello grok"))
    );
    assert_eq!(
        default_state_dir(&cwd, "agent-1"),
        cwd.join(".treer/agents/agent-1")
    );
}
