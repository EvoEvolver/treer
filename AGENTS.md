...
| `crates/treer-acp` | ACP catalog/adapters, Host-local journal, cwd files, Host-wide thread UI dist, and AIS HTTP for one Agent |
| `crates/treer-cli` | Human/Agent commands, Host `treer ui`, and the Core Message surface |
